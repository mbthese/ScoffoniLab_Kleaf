...
#Prepare fitting a nonlinear model
# Estimate the rest parameters using a linear model
model.0.PABI<- lm(log(`Kleaf Tcorr (mmol m-2 s-1 MPa-1)`) ~ `Lowest Psileaf (Mpa)`, data=PABI_Kleaf_clean)  
summary(model.0.PABI)
alpha.0.PABI <- exp(coef(model.0.PABI)[1])
beta.0.PABI <- coef(model.0.PABI)[2]

list(alpha = alpha.0.PABI, beta = beta.0.PABI)
...
#output model
y_model.PABI <-   alpha.0.PABI    * exp(beta.0.PABI   * x_seq.PABI ) 

# Calculate Ψleaf at 50% loss of Kleaf
Psi_leaf_50.PABI <- log(0.5) / beta.0.PABI[[1]]
Psi_leaf_50.PABI
```
...
STDE.plot, DASP.plot, CHLA.plot, ZEMA.plot, NAVI.plot, PABI.plot, nrow=3, ncol=3, labels = c("(a) C. setaceus", "(b) P. dilatatum", "(c) A. sativa", "(d) S. decipiens", "(e) D. spicata", "(f) C. latifolium", "(g) Z. mays ", "(h) N. viridula", "(g) P. bisulcatum"), font.label = list(face="bold.italic"), align = "v")
